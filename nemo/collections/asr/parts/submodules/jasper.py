            ...
            for i, layer in enumerate(self.res):
                ...
                if self.residual_mode == 'add' or self.residual_mode == 'stride_add':
                    if PYTORCH_QUANTIZATION_AVAILABLE and self.quantize:
                        ...
                        out = (out[0] + res_out, out[1])
                else:
                    out = (torch.max(out[0], res_out), out[1])

        # compute the output
        out = (self.mout(out[0]), out[1])
        len_indexes = torch.from_numpy(np.arange(0, lens.shape[-1], n_in)).type(torch.LongTensor)
        fin_lens = lens[len_indexes]
        if self.res is not None and self.dense_residual:
            return xs.view(b_in * n_in, d_in, t_in) + [out[0]], fin_lens
        bn_out, d_out, t_out = out[0].size()

        if self.end_pool:
            features = out[0].view((b_in, n_in, d_out, t_out))
            max_global_features = self.pooling(features)
            out = (max_global_features.squeeze(1), out[1])

        if not self.end_pool and not self.single_channel:
            return [out[0].view(b_in, n_in, d_out, t_out)], fin_lens
        else:
            return [out[0]], fin_lens
